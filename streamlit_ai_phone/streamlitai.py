...
with st.sidebar:
    ...
    st.session_state.system_message = st.text_area("System Message", value=st.session_state.system_message,
                                                   disabled=st.session_state.call_active)
    st.session_state.initial_message = st.text_area("Initial Message", value=st.session_state.initial_message,
                                                    disabled=st.session_state.call_active)
        ...
        with st.spinner(f"Calling {phone_number}..."):
            try:
                ...
                if call_sid := call_data.get('call_sid'):
                    ...
                    for _ in range(60):
                        time.sleep(1)
                        status = requests.get(f"https://{os.getenv('SERVER')}/call_status/{call_sid}").json().get(
                            'status')
    ...
    if end_call:
        try:
            response = requests.post(f"https://{os.getenv('SERVER')}/end_call",
                                     json={"call_sid": st.session_state.call_sid})
...
with st.spinner("Loading recording and transcript..."):
    ...
    if st.session_state.call_active and st.session_state.call_sid:
        ...
        if transcript := next((t for t in st.session_state.all_transcripts if
                               f"Call {t['call_sid']}" == st.session_state.call_selector), None):
...
if st.session_state.call_active:
    def update_call_info():
        try:
            status = requests.get(f"https://{os.getenv('SERVER')}/call_status/{st.session_state.call_sid}").json().get(
                'status')
            ...
            transcript_data = requests.get(
                f"https://{os.getenv('SERVER')}/transcript/{st.session_state.call_sid}").json()
